...
public class MenuAlimentosComprarScreen extends BaseScreen {
...
// Configurar el Stack para estar centrado
        ...
        for(Alimento alimento :principal.getAlimentostienda()){
            agregarElementoAGrilla(alimento.getImagen(), alimento.getNombre()+"("+alimento.getPrecio()+"$)", labelStyle,new ClickListener(){
                @Override
                public void clicked(InputEvent event, float x, float y) {
                    if (principal.getJugador().getMonedas() - alimento.getPrecio() < 0) {
                        ...
                        jugador.setMonedas(jugador.getMonedas() - alimento.getPrecio());
                        monedasLabel.setText("Monedas:" + principal.getJugador().getMonedas());
                        if(jugador.getAlimentos().containsKey(alimento.getNombre())){
                            Alimento origen=jugador.getAlimentos().get(alimento.getNombre());
                            origen.setCantidad(origen.getCantidad()+1);

                        }else {
                            Alimento alimento1=new Alimento(alimento.getNombre(),alimento.getDescripcion(),alimento.getSalud(),alimento.getEnergia(),alimento.getImagen(),alimento.getPrecio(),1);
                            jugador.getAlimentos().put(alimento1.getNombre(), alimento1);
                            principal.setSeleccionado(alimento1);
                        }
                    }
                }
            },alimento);
    ...
    private void agregarElementoAGrilla(String imagePath, String nombre, Label.LabelStyle labelStyle,ClickListener click,Alimento alimento) {
        ...
        descripcionButton.addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
                ((PantallaCaracteristicas)Pantallas.CARACTERISTICAS_OBJETO.getPantalla()).setAlimento(alimento);
